...
function getEmailValidationError(email: string) {
	...
	if (!normalizedEmail) {
		return "Informe o e-mail da conta ativa para continuar com a recuperação de senha.";
	}

	if (normalizedEmail.length > 254) {
		return "O e-mail informado é muito longo. Verifique se foi digitado corretamente.";
	}
	...
	if (atCount !== 1) {
		return "Digite um e-mail válido com apenas um caractere @.";
	}

	const [localPart, domainPart] = normalizedEmail.split("@");

	if (!localPart || !domainPart) {
		return "Digite o endereço completo no formato nome@dominio.com.";
	}

	if (localPart.startsWith(".") || localPart.endsWith(".")) {
		return "A parte antes do @ não pode começar ou terminar com ponto.";
	}

	if (domainPart.startsWith(".") || domainPart.endsWith(".")) {
		return "O domínio do e-mail não pode começar ou terminar com ponto.";
	}
	...
	if (!domainPart.includes(".")) {
		return "O domínio do e-mail precisa conter pelo menos um ponto, como .com ou .br.";
	}

	const domainLabels = domainPart.split(".");

	if (domainLabels.some((label) => !label)) {
		return "O domínio do e-mail contém partes vazias. Revise o endereço.";
	}

	if (domainLabels.some((label) => label.startsWith("-") || label.endsWith("-"))) {
		return "O domínio do e-mail não pode começar ou terminar com hífen.";
	}

	const topLevelDomain = domainLabels[domainLabels.length - 1];

	if (!/^[a-z]{2,}$/i.test(topLevelDomain)) {
		return "O final do domínio parece inválido. Use algo como .com, .com.br ou .gov.br.";
	}
	...
	async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			showPopup({
				type: "success",
				title: "Link enviado",
				message: "Se a conta estiver ativa e o e-mail existir no sistema, um link de redefinição será enviado com validade de 2 horas.",
			});
		} catch {
